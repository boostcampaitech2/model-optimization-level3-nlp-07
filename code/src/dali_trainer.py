...
class DaliTrainer:
    ...
    def train(
        ...
        Returns:
            loss and accuracy
        """
        wandb.init(project="dali experiments",entity="barleysack")
        ...
        for epoch in range(n_epoch):
            ...
            for batch,inputs in pbar:
                    
                
                image = inputs[0]
                label = inputs[1]
                image= image.permute(0,3,1,2).half()
                ...
                train_f1_macro = f1_score(y_true=gt, y_pred=preds, labels=label_list, average='macro', zero_division=0)
            pbar.close()
            wandb.log({
                    'train loss': train_loss, 'train acc': train_acc, 'train_f1_macro': train_f1_macro, 
                })
    ...
    def test(
        ...
        if type(test_dataloader) != "DataLoader":
        ...
        for batch,inputs in enumerate(test_dataloader):
            ...
            if type(test_dataloader) == "DataLoader":  
                data, labels = inputs[0].to(self.device),inputs[1].to(self.device)
                
                
            else:
                data = inputs[0].to(self.device)
                labels = inputs[1].to(self.device)
                data= data.permute(0,3,1,2).half()
        ...
        val_loss = running_loss/(batch+1)
        val_acc = (correct/total)*100
        val_f1_macro = f1_score(y_true=gt, y_pred=preds, labels=label_list, average='macro', zero_division=0)
        pbar.close()
        wandb.log({
                    'val loss': val_loss, 'val acc': val_acc, 'val_f1_macro': val_f1_macro, 
                })
